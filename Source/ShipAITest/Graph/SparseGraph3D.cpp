...
//----------------------------------------------------------------------------
const class NavGraphNode&  SparseGraph3D::GetNode(int32 Idx)const
{
...
//non const version
NavGraphNode&  SparseGraph3D::GetNode(int32 Idx)
{
	check((Idx < (int32)Nodes.Num()) &&
		(Idx >= 0) &&
		"<SparseGraph::GetNode>: invalid index");
...
//----------------------------------------------------------------------------
const NavGraphEdge& SparseGraph3D::GetEdge(int32 F, int32 T)const
{
	check((F < Nodes.Num()) &&
		(F >= 0) &&
...
//non const version
NavGraphEdge& SparseGraph3D::GetEdge(int32 F, int32 T)
{
	check((F < Nodes.Num()) &&
		(F >= 0) &&
		Nodes[F].GetIndex() != InvalidNodeIndex &&
...
void SparseGraph3D::AddEdge(NavGraphEdge Edge)
{
...
//------------------------------------------------------------------------
bool SparseGraph3D::UniqueEdge(int32 F, int32 T)const
{
	for (auto Edge : Edges[F])
	{
		if (Edge.GetTo() == T)
...
//------------------------------------------------------------------------
int SparseGraph3D::AddNode(NavGraphNode  Node)
{
...
//  nodes
//------------------------------------------------------------------------
void SparseGraph3D::RemoveNode(int node)
{
